...
description: >-
  ...
  Daily first-mention behavior is the always-on snippet
  (Cursor rule, CLAUDE.md, AGENTS.md, GEMINI.md), not this skill.
---
...
日常遵守靠各 IDE 的常驻说明，不是本 skill：

- 正文：`always-on.md`（Claude Code / Codex / Gemini / Copilot / 其它工具往各自常驻文件里贴）
- Cursor 封装：`cursor-rule.mdc`（`alwaysApply: true`）

本 skill 只统一格式和反例；不要把它当成每条回复都要加载的流程。
